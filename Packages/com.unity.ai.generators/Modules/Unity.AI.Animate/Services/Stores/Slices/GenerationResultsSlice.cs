    ...
    static class GenerationResultsSlice
    {
        public static void Create(Store store) => store.CreateSlice(
            ...
            reducers => reducers
                ...
                .Add(GenerationResultsActions.setGeneratedAnimations, (state, payload) => {
                    var results = state.generationResults.Ensure(payload.asset);
                    results.generatedAnimations = payload.animations.ToList();
                })
            ...
            state => state with {
                generationResults = new SerializableDictionary<AssetReference, GenerationResult>(
                    state.generationResults.ToDictionary(kvp => kvp.Key, entry => entry.Value with {
                        generatedAnimations = entry.Value.generatedAnimations,
        ...
        internal static void PruneFulfilledSkeletonsReducer(GenerationResults state, AsssetContext payload)
        {
            ...
            var animationUris = new HashSet<string>(results.generatedAnimations.Where(animation => animation.uri != null)
                .Select(animation => animation.uri.GetAbsolutePath()));

            var fulfilledTaskIds = results.fulfilledSkeletons.Where(fs => animationUris.Contains(fs.resultUri))
