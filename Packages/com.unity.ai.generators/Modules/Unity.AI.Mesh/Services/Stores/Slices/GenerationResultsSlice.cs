    ...
    static class GenerationResultsSlice
    {
        public static void Create(Store store) => store.CreateSlice(
            ...
            reducers => reducers
                ...
                .Add(GenerationResultsActions.setGeneratedMeshes, (state, payload) => {
                    var results = state.generationResults.Ensure(payload.asset);
                    results.generatedMeshes = payload.meshes.ToList();
                })
            ...
            state => state with {
                generationResults = new SerializableDictionary<AssetReference, GenerationResult>(
                    state.generationResults.ToDictionary(kvp => kvp.Key, entry => entry.Value with {
                        generatedMeshes = entry.Value.generatedMeshes,
        ...
        static void PruneFulfilledSkeletonsReducer(GenerationResults state, AsssetContext payload)
        {
            var results = state.generationResults.Ensure(payload.asset);

            var meshUris = new HashSet<string>(
                results.generatedMeshes
                    .Where(mesh => mesh.uri != null)
                    .Select(mesh => mesh.uri.GetAbsolutePath())
            );

            var skeletonsToRemove = results.fulfilledSkeletons
                .Where(skeleton => meshUris.Contains(skeleton.resultUri))
                .Select(skeleton => skeleton.progressTaskID)
                .ToHashSet();

            if (skeletonsToRemove.Count == 0)
                return;
